  ...
  <a-drawer :title="form.id ? '编辑' : '添加'" :width="600" :open="visible" :body-style="{ paddingBottom: '80px' }"
    ...
    <a-form ref="formRef" :model="form" :rules="rules" layout="horizontal" :label-col="{ span: 4 }"
      ...
      <a-form-item label="上级部门" name="pid" v-if="form.pid != 0">
        <DepartmentTreeSelect ref="departmentTreeSelect" v-model:value="form.pid" :defaultValueFlag="false"
          width="100%" />
      </a-form-item>
      ...
      <a-form-item label="部门负责人" name="managerId">
        <EmployeeSelect ref="employeeSelect" placeholder="请选择部门负责人" width="100%" v-model:value="form.managerId"
          :leaveFlag="false" />
      </a-form-item>
