...
<template>
  <div class="wrapper">
    <div class="sider-fl">
      <a-tree :tree-data="treeData" :fieldNames="{ title: 'name', key: 'deptId' }" :selectable="false" v-model:expandedKeys="expandedKeys">
        <template #switcherIcon="{ switcherCls }">
          <caret-down-outlined :class="switcherCls" />
        </template>
        <template #title="{ name, deptId }">
          <div class="list-item" :class="{ active: checkExists(deptId) }">
            <div class="list-item-title">{{ name }}</div>
            <check-circle-filled class="check-icon-style" @click="onSelectAdd(name, deptId)" />
          </div>
        </template>
      </a-tree>
    </div>
