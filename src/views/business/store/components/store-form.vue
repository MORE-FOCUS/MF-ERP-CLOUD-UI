...
-->
<template>
  <a-modal :title="form.id ? '编辑' : '添加'" width="400" :visible="visible" @cancel="onClose" :maskClosable="false" :destroyOnClose="true">
...
<script setup>
  ...
  const formDefault = {
    id: undefined,
    id: undefined, //分类id
    isDisabled: undefined, //是否禁用 1-禁用 0-启用
  ...
  const rules = {
    ...
    isDisabled: [{ required: true, message: '是否禁用 1-禁用 0-启用 必填' }],
