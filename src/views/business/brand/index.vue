  ...
  <a-card size="small" :bordered="false" :hoverable="true">
    <a-row class="smart-table-btn-block">
      ...
      <div class="smart-table-setting-block">
        <TableOperator v-model="columns" :tableId="TABLE_ID_CONST.BUSINESS.BRAND" :refresh="queryData" />
      </div>
    </a-row>
    <!---------- 表格 begin ----------->
    <a-table
    ...
    <BrandFormModel ref="brandFormModelRef" @reloadList="queryData" />
  </a-card>
</template>
<script setup>
  ...
  import BrandFormModel from './components/brand-form-model.vue';
  const columns = ref([
    {
      title: '序号',
      dataIndex: 'no',
  ...
  async function queryData() {
    ...
    try {
      let queryResult = await brandApi.queryPage(queryForm);
  ...
  const brandFormModelRef = ref();

  function showForm(data) {
    brandFormModelRef.value.showDrawer(data);
  }
  ...
  async function requestBatchDelete() {
    try {
      SmartLoading.show();
      await brandApi.batchDelete(selectedRowKeyList.value);
      message.success('删除成功');
      queryData();
    } catch (e) {
      smartSentry.captureError(e);
  ...
  function updateDisabled(id, isDisabled) {
    Modal.confirm({
      ...
      async onOk() {
        SmartLoading.show();
        try {
          await brandApi.updateDisabled(id);
